...
st.sidebar.info("Model Strategy: **LSTM** selected based on performance metrics vs alternative RNNs.")
...
if st.button("Generate Forecast"):
    with st.spinner(f"Running {model.name} Analysis..."):
    ...
    if final_price is not None:
        ...
        ax.set_title(f"{selected_org} Price Projection ({model.name})")
        ax.set_ylabel("Price (₦)")
        ax.legend()
